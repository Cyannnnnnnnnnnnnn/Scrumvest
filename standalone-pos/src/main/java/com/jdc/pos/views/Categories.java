...
public class Categories extends AbstractController {
    ...
    private void save(Category category) {
        service.save(category, usuario.getAccount());
        reload();
    }

    private void reload() {
        container.getChildren().clear();
        service.findAllByUser(usuario.getAccount())
                .forEach(category -> container.getChildren().add(new CategoryItem(category, this::save)));
    }
